...
import { CONDITION_LABELS, STATUS_LABELS, BookCondition, BookStatus, Book } from "@/types/book";
...
export function EditBookForm({ book, className }: EditBookFormProps) {
  ...
  const [schoolId, setSchoolId] = useState<number | null>(book.school.id);
  ...
  useEffect(() => {
    setValue("condition", book.condition as BookCondition);
    setValue("status", book.status as BookStatus);
  }, [book, setValue]);
    ...
    try {
      ...
      if (pendingImages.length > 0) {
        try {
          ...
          setImageError(t("books.imageUploadUpdateFailed"));
  ...
  return (
    ...
    >
      ...
      {/* Status */}
      <div className="space-y-2">
        <label className="text-sm font-medium">{t("books.fields.status")}</label>
        <Select
          value={selectedStatus}
          onValueChange={(value) =>
            setValue("status", value as BookStatus)
          }
        >
          <SelectTrigger>
            <SelectValue placeholder={t("books.fields.selectStatus")} />
          </SelectTrigger>
          <SelectContent>
            <SelectGroup>
              {STATUSES.map((status) => (
        ...
        {(formErrors.status || fieldErrors.status) && (
          <FormMessage>{(formErrors.status?.message || fieldErrors.status) as string}</FormMessage>
        )}
      </div>
      ...
      {/* Image Uploader */}
      <div className="space-y-2">
        <label className="text-sm font-medium">{t("books.fields.addImages")}</label>
        <ImageUploader
        ...
        {book.images.length > 0 && (
          <p className="text-sm text-muted-foreground">
            {t("books.fields.existingImages", { count: book.images.length })}
          </p>
        )}
      </div>
